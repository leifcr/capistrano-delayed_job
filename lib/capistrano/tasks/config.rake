...
namespace :load do
  task :defaults do
    ...
    # runit templates
    set :delayed_job_runit_run_template, File.join(TemplatePaths.template_base_path, 'runit', 'run.erb')
    set :delayed_job_runit_finish_template, File.join(TemplatePaths.template_base_path, 'runit', 'finish.erb') # rubocop:disable Metrics/LineLength
    set :delayed_job_runit_log_run_template, File.join(TemplatePaths.template_base_path, 'runit', 'log', 'run.erb') # rubocop:disable Metrics/LineLength
    ...
    set :delayed_job_monit_config_template, File.join(TemplatePaths.template_base_path, 'monit', 'delayed_job.conf.erb') # rubocop:disable Metrics/LineLength
  end
end
